...
- TypeScript（以下TS）は、JSとの互換性を保ちつつ、JSを拡張して作った言語である。よって、JSのコードはすべてTSとしてあつかえる
...
Union型は、変数が取り得る型が複数ある場合に使う型である。ある変数が文字列の「Hello」または「Goodbye」のみを取り得るとき、その変数の型は'Hello' | 'Goodbye'と表現できます。しかし、これらの値が多くなると、手動でUnion型を定義するのは大変である
