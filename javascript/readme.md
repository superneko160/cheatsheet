...
プログラミング言語のひとつです。`JS`と略されます。ほかのプログラミング言語の**Javaとはまったくの別物なのでJavaScriptのことを絶対にJavaと略してはいけません。**現場に混乱を招きます。
...
（`const`と`let`は用意するという意味なので、それ以降で変数を利用する場合は、`const`と`let`は不要です）
...
> ```
> 
> 数値どうしの計算を行いたい場合は、文字列を数値に変換する必要があります。
> 
> ```js
...
```js
const numbers = [11, 7, 52, 24];

// 昇順（値が小さい順）にソート
numbers.sort();
console.log(numbers);  // [7, 11, 24, 52]

// 降順（値が大きい順）にソート
numbers.reverse();
console.log(numbers);  // [52, 24. 11, 7]
```
...
このボタンが押されとき〇〇が起きる......、キーボードのスペースキーを押したとき〇〇が起きてほしい......。この〇〇に入るの総称がイベントです。
...
async function getAddress() {
    ...
    // fetch()はリクエストを行う関数
    const result = await fetch('https://zipcloud.ibsnet.co.jp/api/search?zipcode=7830060');
    return result;
}
```
...
async function getAddress() {
    try {
        const data = await fetch('https://zipcloud.ibsnet.co.jp/api/search?zipcode=7830060');
        return data;
    } catch (error) {
