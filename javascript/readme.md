...
なお、オブジェクトでは、インデックスではなく **プロパティ名** または **キー** と呼びます。要素は **プロパティ** と呼びます。（配列と同じく要素と呼ばれる場合もあり）
...
これらのことをまとめて **イベント** といいます。たとえば、マウスがクリックされることを「クリックイベントが発生する」と表現します。（イベントが「発火する」という言い方がされることもあります）
...
非同期処理は **複数のタスクを並行して実行する重要な仕組み** です。WebAPIサーバからのデータの取得や、ファイルの読み込みなど、**時間のかかる処理を効率的に扱えます。**  

JSには `async/await` という非同期処理を簡潔に記述できる構文が用意されています。

### `async/await` 基本構文

`async` は関数を非同期関数に変換するキーワードです。  
`async` を `function` の前に付与すると、その関数は非同期処理を実行する用の関数になります。  

`await` は非同期処理の結果が返ってくるまで待機するためのキーワードです。  
呼び出している関数の前に `await` を付与すると、その関数の実行結果が返ってくるまでそれ以降の処理は実行されません。
...
`.json()` メソッドでそれが可能です。`.json()` もまた非同期処理の関数なので、 `await` を使います。
